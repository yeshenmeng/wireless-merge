...
/* LORA通信测试数据协议 */
static void lora_load_comm_test_cmd_buf(void)
{
	lora_tx_count = 0;
	
	/* 命令头 */
	uint32_t cmdHeader = CMD_TEST;
#if (COMM_TRANSMISSION_FORMAT == 1)
	cmdHeader = swap_htonl(cmdHeader);
#endif
	memcpy(&lora_tx_buf[lora_tx_count], (uint8_t*)&cmdHeader, sizeof(cmdHeader));
	...
	lora_tx_buf[lora_tx_count] = llrt_mod->payload_len;
	lora_tx_count += 1;
...
static void lora_status_active(void)
{
	...
	if(lora_obj.state == LORA_IDLE)
	{
		...
		{
			lora_load_comm_test_cmd_buf();
		}
		else if(lora_conn_status == LORA_OFFLINE)
		{
	...
	if(lora_cad_detect() == SET)
	{
		...
		else
		{
			...
			if(param->object_version == INCLINOMETER_VERSION)
			{
				rx_air_time = wireless_drv.radio_time_on_air_get(collapse_max_wait_reply_len);
			}
			else if(param->object_version == COLLAPSE_VERSION)
			{
				rx_air_time = wireless_drv.radio_time_on_air_get(clinometer_max_wait_reply_len);
			}
...
static void lora_status_tx_success(void)
{
	...
	if(wirelessCommSvc->parseMasterMsg() == 1)
	{
		...
		if(llrt_status_get() == LLRT_RUN)
		{
			...
			llrt_rx_add();
			lora_out_status = LORA_OUT_STATE_TEST;
...
static void lora_status_tx_fail(void)
{
	...
	uint32_t tx_fail_max_times = lora_obj.param.tx_max_fail_times;
	if(llrt_status_get() == LLRT_RUN && llrt_comm_ctrl_status() == LLRT_CTRL_START)
	{
		tx_fail_max_times = 1;
	}
	
	if(lora_obj.param.tx_fail_times >= tx_fail_max_times)
	{
...
static void lora_status_stop(void)
{
	...
	else if(lora_pre_conn_status == LORA_OFFLINE)
	{
...
static void lora_task_operate(void)
{
	if(lora_obj.state == LORA_IDLE)
	{
		if(llrt_status_get() == LLRT_ACTIVE)
		{
			slp_reset();
			llrt_start();
			lora_state = LORA_ACTIVE;
